import type { GameState, GameCommand } from "@dealopoly/game-engine";
import { getPlayerTableAssets, calculateTotalAssetValue } from "@dealopoly/game-engine";
  ...
  public static getNextBotAction(state: GameState, botPlayerId: string): GameCommand | null {
    ...
    if (state.turn.phase === "action") {
      if (state.turn.actionsRemaining > 0 && bot.hand.length > 0) {
        ...
        // Priority B: Play Wild Property card
        const wildCard = bot.hand.find((c) => c.type === "property-wild");
        if (wildCard) {
          const chosenColor =
            wildCard.primaryColor === "all"
              ? "dark-blue"
              : wildCard.primaryColor ?? "brown";
          return {
            type: "play_property",
            playerId: botPlayerId,
